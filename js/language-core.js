...
<strong>Sales:</strong> Atmosfera International Sales`,
        ...
        "pub13.abstract": "This paper proposes media stratigraphy as a methodology for analyzing the temporal structure of audiovisual media, distinct from the content-focused approach of traditional media analysis. Drawing on geological metaphors of sedimentation, media stratigraphy examines the internal organization of time\u2014rhythms, repetitions, and cuts\u2014rather than semantic meaning. The study reviews the evolution of spatiotemporal visualization techniques, structuring them into two primary domains: two-dimensional strip imaging (slit-scans, movie barcodes, etc.) and three-dimensional video volumes (video-cubes and their operations). Through an analysis of seminal works by artists and researchers, the paper identifies a lack of standardized terminology that hinders methodological consistency. In response, it proposes a taxonomy of spatiotemporal operations, classifying techniques into spacetime capturing, slit extraction, time displacement, and volumetric manipulation. Ultimately, the paper argues that these stratigraphical tools allow scholars to visualize and interpret the structural \"sediment\" of digital media, revealing patterns invisible to standard observation.",
        "dl.proceedings": "Access the proceedings",
        "dl.proceeding": "Access the proceeding",
        "dl.paper_pdf": "Download paper (PDF)",
        "dl.preprint_pdf": "Download preprint (PDF)",
        "dl.book_pdf": "Download book (PDF)",
        "dl.access_paper": "Access the paper",
        "dl.dissertation_pt": "Download dissertation (PT, PDF)",
        "dl.english_translation": "English translation (PDF)",
        "dl.article_pdf": "Download article (PDF)",
        "dl.article_es_pdf": "Download article (ES, PDF)"
...
<strong>Ventas:</strong> Atmosfera International Sales`,
        ...
        "pub13.abstract": "Este artículo propone la estratigrafía de medios como una metodología para analizar la estructura temporal de los medios audiovisuales, distinta del enfoque centrado en el contenido del análisis mediático tradicional. Inspirándose en metáforas geológicas de sedimentación, la estratigrafía de medios examina la organización interna del tiempo —ritmos, repeticiones y cortes— en lugar del significado semántico. El estudio revisa la evolución de las técnicas de visualización espaciotemporal, estructurándolas en dos dominios principales: la imagen de banda bidimensional (slit-scans, movie barcodes, etc.) y los volúmenes de video tridimensionales (video-cubes y sus operaciones). A través de un análisis de obras fundamentales de artistas e investigadores, el artículo identifica una falta de terminología estandarizada que dificulta la consistencia metodológica. En respuesta, propone una taxonomía de operaciones espaciotemporales, clasificando las técnicas en captura espaciotemporal, extracción de bandas (slit), desplazamiento temporal y manipulación volumétrica. En última instancia, el artículo sostiene que estas herramientas estratigráficas permiten a los académicos visualizar e interpretar el \"sedimento\" estructural de los medios digitales, revelando patrones invisibles a la observación estándar.",
        "dl.proceedings": "Acceder a las actas",
        "dl.proceeding": "Acceder a las actas",
        "dl.paper_pdf": "Descargar artículo (PDF)",
        "dl.preprint_pdf": "Descargar preprint (PDF)",
        "dl.book_pdf": "Descargar libro (PDF)",
        "dl.access_paper": "Acceder al artículo",
        "dl.dissertation_pt": "Descargar disertación (PT, PDF)",
        "dl.english_translation": "Traducción al inglés (PDF)",
        "dl.article_pdf": "Descargar artículo (PDF)",
        "dl.article_es_pdf": "Descargar artículo (ES, PDF)"
    }
};
...
function updatePublicationAbstracts(lang) {
    ...
    // Also update download link labels
    document.querySelectorAll('[data-translate-dl]').forEach(el => {
        const key = el.getAttribute('data-translate-dl');
...
function setLanguage(lang) {
    // Update navigation labels (supports button-based tabs on index)
    const projectsBtn = document.querySelector('.section-btn[data-tab="projects"]');
    const publicationsBtn = document.querySelector('.section-btn[data-tab="publications"]');
    const bioBtn = document.querySelector('.section-btn[data-tab="bio"]');
    if (projectsBtn) projectsBtn.textContent = translations[lang].projects;
    if (publicationsBtn) publicationsBtn.textContent = translations[lang].publications;
    if (bioBtn) bioBtn.textContent = translations[lang].bio;
    ...
    // Update publication abstracts and download labels
